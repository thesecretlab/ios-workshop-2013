...
- (void)insertNewObject:(id)sender
{
    ...
    Device *device = [NSEntityDescription insertNewObjectForEntityForName:[entity name] inManagedObjectContext:context];
    
    // If appropriate, configure the new managed object.
    // Normally you should use accessor methods, but using KVC here avoids the need to add a custom class to the template.
    device.deviceName = @"iPhone";
    device.released = @(2007);
...
- (void)configureCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.detailTextLabel.text = [device.released description];
    cell.imageView.image = [UIImage imageWithData:device.photo];
}
